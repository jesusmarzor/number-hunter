    ...
    return (
        <div className="relative">
            <div className={`relative flex flex-col items-start rounded overflow-hidden`}>
                <input onChange={e => setText(e.currentTarget.value)} name={name} value={text} type={type} placeholder={title} className={`peer indent-1.5 leading-10 text-black-default px-0.5 pt-2 placeholder-shown:py-2 w-full h-10 border-2 outline-none placeholder-transparent tracking-wider ${isError ? "border-red-default placeholder-shown:border-red-default" : "border-pink-default placeholder-shown:border-transparent"}`} min={defaultMinNumber}/>
            ...
            { isError && <p className="absolute -bottom-6 left-0 text-red-default">{error}</p>}
        </div>
    )
}